...
/*   Updated: 2024/03/01 01:18:38 by aal-samm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	main_buffer[BUFFER_SIZE + 1];
	int			i;
	...
	if (!line)
		return (NULL);
	if (!ft_strchr(main_buffer, '\n'))
		ft_read_to_buffer(fd, main_buffer, &line);
	line = extract_line(&line, main_buffer);
	obtain_remaining(main_buffer);
	if (line[0] == '\0')
	{
		free(line);
		line = NULL;
